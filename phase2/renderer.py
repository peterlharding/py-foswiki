...
Orchestrates the full Phase 2 render pipeline:
  ...
  2. Macro expansion  (%MACRO{params}%)
        ↓
  3. WikiWord auto-linking
        ↓
  4. Foswiki bracket link conversion  [[target][label]]
        ↓
  5. Markdown rendering  (mistune)
        ↓
...
Usage::
    ...
    pipeline = RenderPipeline(settings, db_session, search_service)
...
from .macros import MacroEngine, MacroContext, macro_registry, register_all_builtins
from .wikiword import WikiWordLinker
...
class RenderPipeline:
    ...
    async def render(
        ...
        # We only link plain-text nodes; skip content inside existing <a> tags.
    ...
    def _make_context(self, web, topic, topic_id, current_user, raw_content) -> MacroContext:
        ...
        # Attach raw content so %TOC% can read it
        ctx._raw_content = raw_content          # type: ignore[attr-defined]
        # Inject ourselves as the render function for %INCLUDE%
    ...
    def _render_markdown(self, text: str) -> str:
        ...
        # Fallback: wrap in <pre> if mistune is unavailable
